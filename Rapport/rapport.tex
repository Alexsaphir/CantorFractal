	...
	\chapter{\bf Dimension des ensembles auto-similaires}
		...
		\section{Dimension de Hausdorff}
			...
			\begin{proof}
				...
				\end{align*}
			\end{proof}
			On peut, donc, grâce à ce lemme montré cette proposition:
			\begin{prop}
...
\newpage
		\section{Dimension de Hausdorff d'un ensemble auto-similaire}
			...
			De plus, La dimension de Hausdorff de l'ensemble $K$ requiert que $K$ soit un ensemble borné. Or, pour que $K$ soit un ensemble auto-similaire, $K$ doit être compact. Or,un compact est borné. On a donc plus d’ambiguïté sur la définition de cette dimension dans le cas des ensembles auto-similaires.
